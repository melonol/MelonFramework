...
class SetTest extends PHPUnit_Framework_TestCase {
	...
	public function testSetMode() {
		$this->_array = new Set( array(), Set::REPLACE_ABSOLUTE );
		$this->_array->set( 'k', 'v' );
		$this->_array->set( 'k', null );
		$this->assertEquals( $this->_array->get( 'k' ), null );
		
		$this->_array = new Set( array(), Set::REPLACE_NOT );
		$this->_array->set( 'k', 'v' );
		$this->_array->set( 'k', 'v1' );
		$this->_array->set( 'k', null );
		$this->assertEquals( $this->_array->get( 'k' ), 'v' );
		
		$this->_array = new Set( array(), Set::REPLACE_RELAXED );
		$this->_array->set( 'k', 'v' );
		$this->_array->set( 'k', 'v1' );
		$this->_array->set( 'k', null );
	...
	public function testSetItems() {
		...
		$this->assertEquals( $this->_array->getItems(), $items );
		$this->assertEquals( $this->_array->getItems( array( 'k1', 'k3' ) ), array(
			'k1' => 'v1',
			'k3' => 'v3',
		) );
	}
	...
	public function testKeys() {
		...
		$this->_array->setItems( $items );
		$this->assertEquals( $this->_array->keys(), array( 'k1', 'k2', 'k3' ) );
	}
	...
	public function testIterator() {
		...
		foreach( $items as $key => $value ) {
			...
			$this->assertTrue( in_array( $key, $this->_array->keys() ) );
		}
	}
